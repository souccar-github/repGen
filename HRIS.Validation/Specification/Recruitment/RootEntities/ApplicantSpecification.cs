...
{
    public class ApplicantSpecification : Validates<Applicant>
    {
        public ApplicantSpecification()
        {
           ...
           // Check(x => x.DateOfBirth).Required().LessThan(DateTime.Now.AddYears(-GlobalConstant.MinimumEmployeeAge));
            ...
            //Check(x => x.NationalityIssue).Optional();
            //Check(x => x.OtherNationalityIssue).Optional();
            ...
            Check(x => x.Phone).Optional().MaxLength(GlobalConstant.SimpleStringMaxLength);
            Check(x => x.Mobile).Optional().MaxLength(GlobalConstant.SimpleStringMaxLength);
            ...
            Check(x => x.IdentificationNo).Required().MaxLength(GlobalConstant.SimpleStringMaxLength);
            ...
            Check(x => x.OtherNationality)
                .Optional().Expect((employee, otherNationality) => otherNationality.IsTransient() == false, "")
                .With(x => x.MessageKey = PreDefinedMessageKeysSpecExpress.GetFullKey(PreDefinedMessageKeysSpecExpress.Required))
                .And.Expect((employee, otherNationality) => employee.Nationality.Id != otherNationality.Id, "")
