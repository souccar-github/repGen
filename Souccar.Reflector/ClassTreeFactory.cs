...
namespace Souccar.Reflector
{
    public class ClassTreeFactory
    {
        ...
        private static ClassTree Create(Type type, string parentFullClassName, HashSet<Type> types)
        {
            var result = new ClassTree { Name = type.Name, Type = type };
            if (types.Contains(type))
                return result;
        ...
        private static void AddCollectionProperty(Type type, ClassTree result, PropertyInfo propertyInfo, HashSet<Type> types)
        {
            ...
            if (!propertyInfo.PropertyType.GetGenericArguments()[0].IsEntity())
                result.AddCollectionProperty(new ReferenceProperty { Name = propertyInfo.Name, ClassName = type.Name, ClassTree = Create(propertyInfo.PropertyType.GetGenericArguments()[0], type.FullName, CloneType(types)), PropertyType = propertyInfo.PropertyType.GetGenericArguments()[0] });
            else
        ...
        private static void AddReferenceProperty(Type type, ClassTree result, PropertyInfo propertyInfo, HashSet<Type> types)
        {
            if (!propertyInfo.PropertyType.IsEntity())
                result.AddReferenceProperty(new ReferenceProperty { Name = propertyInfo.Name, ClassName = type.Name, ClassTree = Create(propertyInfo.PropertyType, type.FullName, CloneType(types)), PropertyType = propertyInfo.PropertyType });
            else
