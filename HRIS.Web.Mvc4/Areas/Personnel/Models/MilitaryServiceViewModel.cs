...
{
    public class MilitaryServiceViewModel : ViewModel
    {
public override void CustomizeGridModel(GridViewModel model, Type type, RequestInformation requestInformation)
        {
            model.ViewModelTypeFullName = typeof(MilitaryServiceViewModel).FullName;
            model.Views[0].EditHandler = "MilitaryEditHandler";
            model.Views[0].ViewHandler = "MilitaryViewHandler";
        ...
        public override ActionResult BeforeCreate(RequestInformation requestInformation, string customInformation = null)
        {
            ...
            if (employee.Gender == HRIS.Domain.Personnel.Enums.Gender.Female)
            {
        ...
        public override void AfterValidation(RequestInformation requestInformation, Souccar.Domain.DomainModel.Entity entity, IDictionary<string, object> originalState, IList<Souccar.Domain.Validation.ValidationResult> validationResults, CrudOperationType operationType, string customInformation = null, Entity parententity = null)
        {
            ...
            var militaryService = entity as MilitaryService;
            var type = typeof(MilitaryService);
           
            if (militaryService.Status == HRIS.Domain.Personnel.Enums.MilitaryStatus.Exempt)
            {

                var befor = emp.MilitaryService.Where(x => x.DateOfExemption == militaryService.DateOfExemption).ToList();
                ...
                if (DateTime.Compare( militaryService.DateOfExemption.GetValueOrDefault(),emp.DateOfBirth) <= 0)
                {
                    var prop = type.GetProperty("DateOfExemption");
            ...
            if (militaryService.Status == HRIS.Domain.Personnel.Enums.MilitaryStatus.Delayed)
            {
                ...
                var befor = emp.MilitaryService.Where(x => x.DateOfDelay == militaryService.DateOfDelay).ToList();
                ...
                if (DateTime.Compare(militaryService.DateOfDelay.GetValueOrDefault(), emp.DateOfBirth) <= 0)
                {
                    var prop = type.GetProperty("DateOfDelay");
                ...
                )
            {
                ...
                if (DateTime.Compare(militaryService.MilitiryServiceDocIssuance.GetValueOrDefault(), emp.DateOfBirth) <= 0)
                {
                    var prop = type.GetProperty("MilitiryServiceDocIssuance");
                ...
                if (DateTime.Compare(militaryService.ServiceStartDate.GetValueOrDefault(), emp.DateOfBirth) <= 0)
                {
                    var prop = type.GetProperty("ServiceStartDate");
            ...
            if (militaryService.Status == HRIS.Domain.Personnel.Enums.MilitaryStatus.Hold)
            {
                ...
                var befor = emp.MilitaryService.Where(x => x.HoldDate == militaryService.HoldDate).ToList();
            ...
            if(militaryService.Status== HRIS.Domain.Personnel.Enums.MilitaryStatus.Reserve)
            {
                ...
                var befor = emp.MilitaryService.Where(x => x.ReserveStartDate == militaryService.ReserveStartDate).ToList();
            ...
            if (militaryService.Status == HRIS.Domain.Personnel.Enums.MilitaryStatus.Served)
            {
                 var befor = emp.MilitaryService.Where(x => x.ServiceStartDate == militaryService.ServiceStartDate).ToList();
        ...
        {
            var employee =ServiceFactory.ORMService.GetById<Employee>(requestInformation.NavigationInfo.Previous[0].RowId);
            var militaryService = (MilitaryService)entity;
            employee.MilitaryStatus = militaryService.Status;
        ...
        private void MilitaryServiceStatusEffiected(MilitaryService militaryService)
        {
