...
{
    public class TemplateSectionWeightEventHandlers : ViewModel
    {
        public override void CustomizeGridModel(GridViewModel model, Type type, RequestInformation requestInformation)
        {
             model.ViewModelTypeFullName = typeof(TemplateSectionWeightEventHandlers).FullName;
        ...
        public override void AfterValidation(RequestInformation requestInformation, Entity entity, IDictionary<string, object> originalState, IList<ValidationResult> validationResults, CrudOperationType operationType, string customInformation = null, Entity parententity = null)
        {
            ...
            var templateSectionWeight = (TemplateSectionWeight)entity;
            var appraisalTemplate = (AppraisalTemplate)typeof(AppraisalTemplate).GetById(masterRowId);
            var sumWeight = appraisalTemplate.TemplateSectionWeights.Where(x=>x.Id!=entity.Id).Sum(x => x.Weight);
            sumWeight += templateSectionWeight.Weight;
