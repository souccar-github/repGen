    ...
    public class FluentSessionProvider
    {
            ...
            switch (GetCurrentDBMSProvider())
            {
                case DBMSProvider.SQLServer2008:
                    return Fluently.Configure().Database(
                        ...
                        ConfigurationSettings.AppSettings["DefaultConnection"].ToString())
                        ).Mappings(m => m.FluentMappings.AddFromAssemblyOf<EmployeeMap>().Conventions.Setup(x=>x.Add(AutoImport.Never())).Conventions.Add(new EnumConvention()));
                case DBMSProvider.Oracle:
                    return Fluently.Configure().Database(
                        OracleDataClientConfiguration.Oracle10.ConnectionString(
                            ...
                            Mappings(m => m.FluentMappings.AddFromAssemblyOf<EmployeeMap>().Conventions.Add(new CustomManyToManyTableNameConvention()));
                default:
        ...
        public static bool GenerateSchema(GenerateSchemaMode generateSchemaMode)
        {
                ...
                System.Action<string> updateExport = x =>
                   {
                       using (var file = new System.IO.FileStream(ConfigurationSettings.AppSettings["GenerateSchemaPath"], 
