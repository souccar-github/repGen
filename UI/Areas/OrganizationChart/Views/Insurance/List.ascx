...
<% using (Ajax.BeginForm("Save", "Insurance", new AjaxOptions { OnComplete = "JsonAdd_OnComplete" }))
   {%>
<script type="text/javascript">
    function JsonAdd_OnComplete(context) {
        ...
        if (JsonAdd.Success) {
            var url = '<%: Url.Action("Index", "Grade", new { selectedTabOrder = 0 }) %>';
            self.location = url;
        } else {
...
<div id="addValueObjectArea" style="width: auto">
    <% 
       if (ViewData["ValueObjectsList"] != null & ((IEnumerable<HRIS.Domain.OrgChart.ValueObjects.Insurance>)ViewData["ValueObjectsList"]).Count() == 0)
       {
           if (Model != null)
           {
...
<script type="text/javascript">
    ...
    function add() {
        $('#addValueObjectArea').load('<%: Url.Action("Load", "Insurance") %>');
        Toggle("add");
    }
</script>
