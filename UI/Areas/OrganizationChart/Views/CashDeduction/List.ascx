...
<% using (Ajax.BeginForm("Save", "CashDeduction", new AjaxOptions { OnComplete = "JsonAdd_OnComplete" }))
   {%>
<script type="text/javascript">
    function JsonAdd_OnComplete(context) {
        ...
        if (JsonAdd.Success) {
            var url = '<%: Url.Action("Index", "Grade", new { selectedTabOrder=1 }) %>';
            //$.reLoad(url);
            self.location = url;
        } else {
...
<div id="addValueObjectArea" style="width: auto">
    <% 
       if (ViewData["ValueObjectsList"] != null & ((IEnumerable<HRIS.Domain.OrgChart.ValueObjects.CashDeduction>)ViewData["ValueObjectsList"]).Count() == 0)
       {
           if (Model != null)
           {
...
<script type="text/javascript">
    ...
    function add() {
        $('#addValueObjectArea').load('<%: Url.Action("Load", "CashDeduction") %>');
        Toggle("add");
    }
</script>
