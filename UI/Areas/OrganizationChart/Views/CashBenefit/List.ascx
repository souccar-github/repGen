...
<% using (Ajax.BeginForm("Save", "CashBenefit", new AjaxOptions { OnComplete = "JsonAdd_OnComplete" }))
   {%>
<script type="text/javascript">
    function JsonAdd_OnComplete(context) {
        ...
        if (JsonAdd.Success) {
            var url = '<%: Url.Action("Index", "Grade", new { selectedTabOrder=3 }) %>';
            self.location = url;
        } else {
...
<div id="addValueObjectArea" style="width: auto">
    <% 
       if (ViewData["ValueObjectsList"] != null && ((IEnumerable<HRIS.Domain.OrgChart.ValueObjects.CashBenefit>)ViewData["ValueObjectsList"]).Count() == 0)
       {
           if (Model != null)
           {
...
<script type="text/javascript">
    ...
    function add() {
        $('#addValueObjectArea').load('<%: Url.Action("Load", "CashBenefit") %>');
