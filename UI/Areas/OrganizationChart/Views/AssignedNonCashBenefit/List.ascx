<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<HRIS.Domain.OrgChart.ValueObjects.AssignedGrade.AssignedNonCashBenefit>" %>
<%@ Import Namespace="HRIS.Domain.OrgChart.ValueObjects.AssignedGrade" %>
<% using (Ajax.BeginForm("Save", "AssignedNonCashBenefit", new AjaxOptions { OnComplete = "JsonAdd_OnComplete" }))
   {%>
...
<div id="addValueObjectArea" style="width: auto">
    <% 
       if (ViewData["ValueObjectsList"] != null & ((IEnumerable<AssignedNonCashBenefit>)ViewData["ValueObjectsList"]).Count() == 0)
       {
           if (Model != null)
           {
...
<script type="text/javascript">
    ...
    function add() {
        $('#addValueObjectArea').load('<%: Url.Action("Load", "AssignedNonCashBenefit") %>');
