        ...
        <td style="width: 70%; vertical-align: top">
            <div id="NodePositions">
                <fieldset class="ParentFieldset">
                    <legend class="ParentLegend"><%: Resources.Areas.Services.AssignEmployeeToPosition.AssignEmployeeToPositionModel.EmployeeWeightFieldsetTitle %></legend>
...
<script type="text/javascript">
    ...
    function employeesValueChanged() {
        ...
        if (comboBox.value() != null && comboBox.value() != 0) {
            ...
            $.ajax({
                url: '<%:Url.Action("IsPrimaryPositionFulfillment", "AssignEmployeeToPosition", new { area = "Services"})%>/',
                type: "POST",
    ...
    function AssignEmployeeToPosition() {
        ...
        $.ajax({
            url: '<%: Url.Action("AddPositionFulfillment", "AssignEmployeeToPosition", new { area = "Services"})%>/', type: "POST",
            data: {
    ...
    function FillDataGrid() {
        ...
        $.ajax({
            url: '<%:Url.Action("Index", "PositionFulfillment", new { area = "OrganizationChart"})%>/',
            type: "POST",
            data: { positionId: positionId },
            success: function (result) {
                if (result.Success) {
