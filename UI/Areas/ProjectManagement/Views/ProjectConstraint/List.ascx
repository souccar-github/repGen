<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<HRIS.Domain.ProjectManagment.ValueObjects.ProjectConstraint>" %>

<%@ Import Namespace="HRIS.Domain.ProjectManagment.ValueObjects" %>
<% using (Ajax.BeginForm("Save", "ProjectConstraint", new AjaxOptions { OnComplete = "JsonAdd_OnComplete" }))
   {%>
...
<div id="addValueObjectArea" style="width: auto">
    <% 
       if (ViewData["ValueObjectsList"] != null & ((IEnumerable<ProjectConstraint>)ViewData["ValueObjectsList"]).Count() == 0)
       {
           if (Model != null)
           {
...
<script type="text/javascript">
    ...
    function add() {
        ...
        $('#addValueObjectArea').load('<%: Url.Action("Load", "ProjectConstraint") %>');
        Toggle("add");
    }
