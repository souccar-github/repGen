...
<% using (Ajax.BeginForm("Save", "ProjectKpi", new AjaxOptions { OnComplete = "JsonAdd_OnComplete" }))
   {%>
...
<div id="addValueObjectArea" style="width: auto">
    <% 
       if (ViewData["ValueObjectsList"] != null & ((IEnumerable<ProjectKpi>)ViewData["ValueObjectsList"]).Count() == 0)
       {
           if (Model != null)
           {
...
<script type="text/javascript">
    ...
    function add() {
        ...
        $('#addValueObjectArea').load('<%: Url.Action("Load", "ProjectKpi") %>');
        Toggle("add");
    }
