...
<% using (Ajax.BeginForm("Save", "ProjectSuccessFactor", new AjaxOptions { OnComplete = "JsonAdd_OnComplete" }))
   {%>
...
<div id="addValueObjectArea" style="width: auto">
    <% 
       if (ViewData["ValueObjectsList"] != null & ((IEnumerable<ProjectSuccessFactor>)ViewData["ValueObjectsList"]).Count() == 0)
       {
           if (Model != null)
           {
...
<script type="text/javascript">
    ...
    function add() {
        ...
        $('#addValueObjectArea').load('<%: Url.Action("Load", "ProjectSuccessFactor") %>');
        Toggle("add");
    }
