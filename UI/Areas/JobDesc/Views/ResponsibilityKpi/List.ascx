...
<% using (Ajax.BeginForm("Save", "ResponsibilityKpi", new AjaxOptions { OnComplete = "JsonAdd_OnComplete" }))
   {%> 
...
<div id="addValueObjectArea" style="width: auto">   
    <% 
       if (ViewData["ValueObjectsList"] != null & ((IEnumerable<ResponsibilityKpi>)ViewData["ValueObjectsList"]).Count() == 0)
       {
           if (Model != null)
           {
...
<script type="text/javascript">
    ...
    function add() {
        $('#addValueObjectArea').load('<%: Url.Action("Load", "ResponsibilityKpi") %>');
