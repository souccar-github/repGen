...
<% using (Ajax.BeginForm("Save", "Knowledge", new AjaxOptions { OnComplete = "JsonAdd_OnComplete" }))
   {%>
...
<div id="addValueObjectArea">
    <% 
       if (ViewData["ValueObjectsList"] != null & ((IEnumerable<Knowledge>)ViewData["ValueObjectsList"]).Count() == 0)
       {
           if (Model != null)
           {
...
<script type="text/javascript">
    ...
    function add() {
        $('#addValueObjectArea').load('<%: Url.Action("Load", "Knowledge") %>');
