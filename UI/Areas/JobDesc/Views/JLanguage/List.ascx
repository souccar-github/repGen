...
<% using (Ajax.BeginForm("Save", "JLanguage", new AjaxOptions { OnComplete = "JsonAdd_OnComplete" }))
   {%>
...
<div id="addValueObjectArea">
    <% 
       if (ViewData["ValueObjectsList"] != null & ((IEnumerable<JLanguage>)ViewData["ValueObjectsList"]).Count() == 0)
       {
           if (Model != null)
           {
...
<script type="text/javascript">
    function add() {
        $('#addValueObjectArea').load('<%: Url.Action("Load", "JLanguage") %>');
        Toggle("add");
    }
</script>
