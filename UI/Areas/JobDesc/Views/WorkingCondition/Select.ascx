...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
    <% foreach (var item in (IEnumerable<WorkingCondition>)ViewData["ValueObjectsList"])
       { %>
    <tr style="border-width: 11; border-color: Black; border-spacing: 11">
        <td style="width: 34%; vertical-align: top">
            ...
            <div class="display-field">
                <%: Html.TextAreaFor(model => item.InternalRelationships, new { @readonly = true, @class = "MultiLine" })%>
            </div>
        </td>
        <td style="width: 33%; vertical-align: top">
            ...
            <div class="display-field">
                <%: Html.TextAreaFor(model => item.ExternalRelationships, new { @readonly = true, @class = "MultiLine" })%>
            </div>
        </td>
        <td style="width: 33%; vertical-align: top">
            <% if (ViewData["CanUpdate"] != null && (bool)ViewData["CanUpdate"])
               {%>
            <%: Ajax.ActionLink(Resources.Shared.Buttons.Function.Edit, "JsonEdit", "WorkingCondition", new { Id = item.Id }, new AjaxOptions { OnComplete = "JsonEdit_OnComplete" })%>
            <% } %>
        </td>
    </tr>
    <% } %>
